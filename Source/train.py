...
print("Training labels: %d zeros, %d ones" %
      (zero_labels, nbRatingsTrain-zero_labels))
...
print("Test labels: %d zeros, %d ones" %
      (zero_labels, nbRatingsTest-zero_labels))
...
train_data = writeDatasetToProtobuf(
    X_train, Y_train, bucket, train_prefix, train_key)
test_data = writeDatasetToProtobuf(
    X_test, Y_test, bucket, test_prefix, test_key)
...
if (no_hyper_parameter_tuning):
    ...
    my_tuner.fit({'train': train_data, 'test': test_data},
